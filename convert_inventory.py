...
def create_markdown(files, output_path):
    ...
    markdown_content.append("This document provides an overview of all files in the project repository.")
    markdown_content.append("")
    ...
    for dir_name, dir_files in sorted_dirs[:20]:  # Top 20 directories
        ...
        markdown_content.append(f"| {dir_name} | {format_number(len(dir_files))} | {format_number(dir_loc)} |")
    ...
    markdown_content.append("---")
    markdown_content.append("")
    markdown_content.append("*This file inventory was generated automatically from the repository analysis.*")
    ...
    print(f"Processed {stats['total_files']} files with {format_number(stats['total_loc'])} total lines of code")
