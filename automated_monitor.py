...
class AutomatedMonitor:
        ...
        try:
            ...
            if critical_issues:
                ...
                for issue in critical_issues:
                    logger.error(
                        "automated_monitor",
                        f"Critical: {issue.check_name} - {issue.details}",
                    )
            else:
                logger.info(
                    "automated_monitor",
        ...
        try:
            ...
            if not offline_guard.is_enabled():
                ...
                self._log_correction(
                    "Offline Guard", "Re-enabled offline guard protection"
                )
    ...
    def _monitor_performance(self):
        ...
        try:
            ...
            if memory_mb > self.performance_thresholds["memory_mb"]:
                self._log_alert(
                    "warning", "Performance", f"High memory usage: {memory_mb:.1f}MB"
                )
            ...
            if cpu_percent > self.performance_thresholds["cpu_percent"]:
                self._log_alert(
                    "warning", "Performance", f"High CPU usage: {cpu_percent:.1f}%"
                )
    ...
    def _log_alert(self, severity: str, component: str, message: str):
        ...
        if severity == "critical":
            logger.error(
                "automated_monitor", f"CRITICAL ALERT: {component} - {message}"
            )
    ...
    def get_monitoring_status(self) -> Dict[str, Any]:
        ...
        recent_alerts = [
            ...
            if datetime.fromisoformat(
                alert.timestamp.replace("Z", "+00:00").replace("T", " ")[:19]
            )
        ...
        return {
            ...
            "corrections_applied_today": len(
                [
                    ...
                    if datetime.fromisoformat(c["timestamp"][:19])
                    > datetime.now() - timedelta(days=1)
                ]
            ),
            "system_status": (
                "healthy"
                if len([a for a in recent_alerts if a.severity == "critical"]) == 0
        ...
        return {
            "overall_health": (
                ...
                if health_score >= 0.95
                    ...
                    else "needs_attention" if health_score >= 0.70 else "critical"
                )
            ),
            ...
            "recommendations": self._generate_health_recommendations(
                health_score, monitoring_status
            ),
        }

    def _generate_health_recommendations(
        self, health_score: float, monitoring_status: Dict[str, Any]
    ) -> List[str]:
        ...
        if health_score < 0.8:
            recommendations.append(
                "Run comprehensive system validation to identify and resolve issues"
            )

        if monitoring_status["critical_alerts_24h"] > 0:
            recommendations.append(
                "Address critical alerts immediately for system stability"
            )

        if not monitoring_status["monitoring_active"]:
            recommendations.append(
                "Enable automated monitoring for continuous system health oversight"
            )

        if len(recommendations) == 0:
            recommendations.append(
                "System operating at optimal performance - continue current practices"
            )
