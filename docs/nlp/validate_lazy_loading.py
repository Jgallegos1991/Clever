...
def main():
    ...
    try:
        ...
        print(
            f"Standard loading: {standard['load_time']:.2f}s, {standard['memory_used']:.1f}MB"
        )
        print(f"Lazy loading (creation): {lazy['creation_time']:.4f}s")
        print(
            f"Lazy loading (first use): {lazy['total_time']:.2f}s, {lazy['memory_used']:.1f}MB"
        )
        print(
            f"Minimal pipeline: {minimal['load_time']:.2f}s, {minimal['memory_used']:.1f}MB"
        )
