...
class CleverBackupSystem:
    ...
    def create_full_backup(self) -> str:
        ...
        databases = ["clever.db", "clever_memory.db"]
        for db_name in databases:
            if os.path.exists(db_name):
                shutil.copy2(db_name, db_backup_path / db_name)
                backup_manifest["components"]["databases"] = backup_manifest["components"].get("databases", [])
                backup_manifest["components"]["databases"].append(db_name)
                print(f"  ✅ {db_name}")
        ...
        config_files = [
            "config.py", "user_config.py", "debug_config.py", 
            "requirements.txt", "requirements-base.txt", "Makefile",
            ".env.sample", "jsconfig.json"
        ]
        ...
        core_modules = [
            "app.py", "persona.py", "nlp_processor.py", "database.py",
            "knowledge_base.py", "evolution_engine.py", "file_ingestor.py",
            "debug_config.py", "health_monitor.py", "error_recovery.py",
            "test_suite.py", "enhanced_pdf_sync.py", "core_nlp_logic.py"
        ]
        ...
        if os.path.exists("templates"):
            shutil.copytree("templates", ui_backup_path / "templates", dirs_exist_ok=True)
            print("  ✅ templates/")
        ...
        for data_dir in data_dirs:
            if os.path.exists(data_dir):
                shutil.copytree(data_dir, data_backup_path / data_dir, dirs_exist_ok=True)
        ...
        doc_files = [
            "README.md", "README.copilot.md", "PROJECT_STRUCTURE.md",
            "CLEVER_FRAMEWORK_DEMO.md", "MAGICAL_UI_IMPLEMENTATION.md",
            "PHONE_ACCESS_AND_PDF_SYNC.md", "IMPLEMENTATION_SUMMARY.md"
        ]
        ...
        if os.path.exists("docs"):
            shutil.copytree("docs", docs_backup_path / "docs_folder", dirs_exist_ok=True)
    ...
    def create_database_snapshot(self) -> str:
        """Create detailed database snapshot with metadata"""
        ...
        for db_name in databases:
            if os.path.exists(db_name):
                ...
                snapshot_data["databases"][db_name] = db_info
